...
#include "src/plugins/gres/common/gres_common.h"
 ...
 */
const char	*plugin_name		= "Gres SX Aurora VE and GPU plugin";
...
/*
  Helper functions
*/
static void _replace_commas(char *src)
{
	char *p = src;
	while (*p != '\0') {
		if (*p == ',') *p = ' ';
...
static int _count_ves(char *src)
{
	int ves = 0;
	for (char *p = src; *p != '\0';) {
		ves++;
...
/*
  Env vars for VE
    _VENODELIST: List of VE that we can use.
    VE_NODE_NUMBER: set to first VE in the list.
  
  To allow NEC MPI to run:
    _NECMPI_VE_NODELIST: List of VE that we can use.
    _NECMPI_JOBTYPE: MPI job type set to BATCH
    _NECMPI_VE_NUM_NODES: Number of VE allocated on the VH.
    _NECMPI_VH_NUM_NODES: Number of VH allocated.
    _NECMPI_VH_NODENUM=$SLURM_NODEID
    _NECMPI_VH_NUM_NODES=$SLURM_NNODES
...
#if 0
	// the value of SLURM_NODEID on other nodes of a step is wrong here
	if ((p = getenvp(*env_ptr, "SLURM_NODEID")) != NULL) {
		info("%s SLURM_NODEID=%s", __func__, p);
		env_array_overwrite(env_ptr, "_NECMPI_VH_NODENUM",
				    xstrdup(p));
	}
#endif
	xfree(velist);
		     ...
		     bool reset, bool is_job, gres_internal_flags_t flags)
{
	...
	bool gpu_is_ve = false;

	if (gres_ptr == NULL)
		return;

	debug2("%s: Entering %s", plugin_name, __func__);
	if (is_job) {
		gres_job_state_t *gres_job_ptr = (gres_job_state_t *) gres_ptr;
		debug2("%s job: gres_type=%s", __func__, gres_job_ptr->type_name);
		slurm_env_var = "SLURM_JOB_GPUS";
		if (!xstrncmp("ve", gres_job_ptr->type_name, 2))
			env_array_overwrite(env_ptr, "SLURM_GPUS_ARE_VES", "1");
	} else {
		gres_step_state_t *gres_step_ptr = (gres_step_state_t *) gres_ptr;
		debug2("%s step: gres_type=%s", __func__, gres_step_ptr->type_name);
		slurm_env_var = "SLURM_STEP_GPUS";
	}

	if (getenvp(*env_ptr, "SLURM_GPUS_ARE_VES"))
		gpu_is_ve = true;
...
#ifdef NEC_DEBUG
	info("%s: local_list=%s global_list=%s", plugin_name, local_list, global_list);
#endif
	if (global_list) {
	...
	debug2("%s: Exiting %s", plugin_name, __func__);
}
			   ...
			   gres_epilog_info_t *epilog_info, int node_inx)
{
	...
	if (dev_num_str) {
		...
		if (getenvp(*epilog_env_ptr, "SLURM_GPUS_ARE_VES")) {
			_replace_commas(dev_num_str);
			xstrfmtcat((*epilog_env_ptr)[env_inx++],
				   "_VENODELIST=%s", dev_num_str);
			xstrfmtcat((*epilog_env_ptr)[env_inx++],
				   "VEDA_VISIBLE_DEVICES=%s",
				   dev_num_str);
		}
		xfree(dev_num_str);
	}
